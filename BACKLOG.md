...
- 🟡 **موتور AI** (پیش‌بینی قیمت، توصیه‌گر طراحی، Matching خریدار/فروشنده) — artifactهای قابل‌آموزش برای رگرسیون قیمت، popularity/collaborative baseline و matcher اضافه شد؛ workspace برای کاربر عادی از `/me` و برای مدیر از گزارش‌های مدیریتی استفاده می‌کند؛ سرویس AI محلی در استک E2E با تست `/health` و `/predict-price` پوشش داده شده است؛ اتصال داده‌های واقعی، آموزش دوره‌ای production و providerهای پیشرفته هنوز باقی است
- 🟡 **جامعه + چالش‌های طراحی** (صفحه‌ی عمومی، لایک/کامنت/ذخیره، چالش ماهانه، رأی‌گیری، جوایز/نشان خودکار) — feed عمومی، انتشار طرح، like، comment، ذخیره/حذف ذخیره با کنترل مالکیت، moderation پنل ادمین، نمایش چالش‌ها، ثبت idempotent جایزه/نشان برنده و credit اتمیک reward ریالی به کیف پول متصل شد؛ چرخه‌ی کامل badge/reward غیرمالی و تجربه‌ی عمومی جوایز هنوز باقی است
