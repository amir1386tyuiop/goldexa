...
- 🟡 **جامعه + چالش‌های طراحی** (صفحه‌ی عمومی، لایک/کامنت/ذخیره، چالش ماهانه، رأی‌گیری، جوایز/نشان خودکار) — feed عمومی، انتشار طرح، like، comment، ذخیره/حذف ذخیره با کنترل مالکیت، moderation پنل ادمین، نمایش چالش‌ها و ثبت idempotent جایزه/نشان برنده به frontend متصل شد؛ پرداخت واقعی جایزه و چرخه‌ی کامل badge/reward هنوز باقی است
