...
- `POST /recommend-designs` با `user_id` و `user_history` ویژگی‌های مشترک سابقه و طرح را می‌شمارد. برای ارائه‌ی catalog می‌توان `candidate_designs` شامل `product_id`، `name` و `tags` فرستاد؛ در غیر این صورت catalog نمونه‌ی داخلی استفاده می‌شود.
