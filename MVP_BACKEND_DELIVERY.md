...
- **کش in-memory** (TTL 15s)، **اعتبارسنجی نوسان** (رد جهش بیش از ۲٪)، **ثبت `price_history`** (append هنگام تغییر).
...
- چرخه‌ی payout اکنون مرحله‌ی صریح `processing` هم دارد: پس از تأیید ادمین، برداشت وارد تسویه بانکی می‌شود و فقط سپس با شماره مرجع به `paid` منتقل می‌شود.
...
- آپلود تصویر محصول پس از چند مرحله resize/quality فقط وقتی ذخیره می‌شود که خروجی WebP حداکثر ۲۰۰KB باشد؛ خروجی بزرگ‌تر با خطای validation رد می‌شود.
